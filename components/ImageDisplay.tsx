...
const ImageCard: React.FC<{ image: ImageData; index: number; onDelete: () => void; }> = ({ image, index, onDelete }) => {
  return (
    <div className="aspect-square bg-slate-100 rounded-lg overflow-hidden relative group ring-1 ring-slate-200 hover:ring-cyan-400 transition-all">
      <img 
...
const AddImageCard: React.FC<{ onClick: () => void }> = ({ onClick }) => (
  <button
    onClick={onClick}
    className="aspect-square flex flex-col items-center justify-center border-2 border-dashed border-slate-300 rounded-lg text-slate-500 hover:border-cyan-400 hover:text-cyan-400 transition-colors duration-300"
    aria-label="添加新图片"
  >
...
const ImageDisplay: React.FC<ImageDisplayProps> = ({ images, onDeleteImage, onAddImages }) => {
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className={`bg-white p-6 rounded-2xl shadow-lg min-h-[120px] transition-all duration-300 ring-1 ${isDragging ? 'ring-cyan-400' : 'ring-slate-900/5'}`}
    >
      {images.length === 0 ? (
        <label 
            htmlFor="file-upload-display"
            className="flex flex-col items-center justify-center h-full cursor-pointer text-center text-slate-500 hover:text-cyan-500 transition-colors"
        >
